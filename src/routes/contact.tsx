...
function Contact() {
  ...
  function goToReview() {
    ...
    // Admins are exempt from the client-side spam brake so they can submit
    // unlimited test/internal briefs. Public visitors keep the normal limit.
    const rateLimit = isAdmin ? { allowed: true as const } : checkContactRateLimit();
  ...
  async function confirmAndSend() {
    ...
    try {
      ...
      if (!isAdmin) recordContactSubmission();
      setStep("sent");
    } catch (error) {
      toast.error(
