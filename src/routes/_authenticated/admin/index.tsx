...
import { AlertTriangle, CheckCircle2, Inbox, Loader2, MailCheck, Send, Users } from "lucide-react";

import { supabase } from "@/integrations/supabase/client";
import { getEmailStatus, sendResendTestEmail } from "@/lib/contact.functions";
...
function AdminOverview() {
  ...
  const resendTest = useServerFn(sendResendTestEmail);
  ...
  const testEmail = useMutation({
    mutationFn: () => resendTest({ data: undefined }),
  });
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="rounded-4xl border border-border bg-surface-1 p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            type="button"
            disabled={!email?.configured || testEmail.isPending}
            onClick={() => testEmail.mutate()}
            className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
          >
            {testEmail.isPending ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
            Send test email
          </button>
        </div>
        <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
          {email?.configured
            ? "Resend is connected. The test button sends directly to the current admin test inbox using Resend's testing sender."
            : "Resend isn't configured yet. Add RESEND_API_KEY to the Vercel Production environment and redeploy."}
        </p>
        {testEmail.data && (
          <pre className="mt-4 overflow-x-auto rounded-2xl border border-border bg-card p-4 text-xs whitespace-pre-wrap">
            {JSON.stringify(testEmail.data, null, 2)}
          </pre>
        )}
        {testEmail.error && (
          <pre className="mt-4 overflow-x-auto rounded-2xl border border-destructive/30 bg-card p-4 text-xs whitespace-pre-wrap text-destructive">
            {testEmail.error instanceof Error ? testEmail.error.message : String(testEmail.error)}
          </pre>
        )}
      </section>
    </div>
