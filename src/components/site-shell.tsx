...
import { ArrowRight, LayoutDashboard, LogIn, LogOut, Menu, UserPlus, X } from "lucide-react";
    ...
    return (
      <div className="flex items-center gap-1.5">
        <Link
          to="/auth"
          search={{ mode: "signin" }}
          className="state-layer inline-flex items-center gap-2 rounded-full border border-outline px-4 py-2.5 text-sm font-semibold"
        >
        ...
        <Link
          to="/auth"
          search={{ mode: "signup" }}
          className="state-layer hidden items-center gap-2 rounded-full bg-primary-container px-4 py-2.5 text-sm font-semibold text-on-primary-container sm:inline-flex"
        >
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Link
        ...
        className="state-layer inline-flex items-center gap-2 rounded-full bg-primary-container px-4 py-2.5 text-sm font-semibold text-on-primary-container"
      >
    ...
    return (
      <div className="mt-3 grid grid-cols-2 gap-2">
        <Link
          ...
          search={{ mode: "signin" }}
          onClick={onNavigate}
          className="state-layer inline-flex items-center justify-center gap-2 rounded-full border border-outline px-4 py-3 text-sm font-semibold"
        >
        ...
        <Link
          ...
          search={{ mode: "signup" }}
          onClick={onNavigate}
          className="state-layer inline-flex items-center justify-center gap-2 rounded-full bg-primary-container px-4 py-3 text-sm font-semibold text-on-primary-container"
        >
          <UserPlus size={15} /> Sign up
        </Link>
      </div>
  ...
  return (
    <div className="mt-3 grid grid-cols-2 gap-2">
      <Link
        ...
        className="state-layer inline-flex items-center justify-center gap-2 rounded-full bg-primary-container px-4 py-3 text-sm font-semibold text-on-primary-container"
      >
        <LayoutDashboard size={15} /> {isAdmin ? "Admin" : "My account"}
      </Link>
      <button
        type="button"
        onClick={() => {
        ...
        className="state-layer inline-flex items-center justify-center gap-2 rounded-full border border-border px-4 py-3 text-sm font-semibold text-muted-foreground"
      >
...
function Brand({ onClick }: { onClick?: () => void }) {
  return (
    <Link to="/" onClick={onClick} className="group flex items-center gap-3">
      <span className="grid size-10 place-items-center rounded-2xl bg-primary font-display text-lg font-bold text-primary-foreground transition-transform duration-500 ease-[cubic-bezier(0.2,0,0,1)] group-hover:rotate-[-8deg]">
        G
      </span>
    ...
    <>
      <motion.header
        ...
        className={cn(
          ...
          scrolled ? "bg-background/85 shadow-elev-1 backdrop-blur-xl" : "bg-transparent",
        )}
      >
...
export function PageTransition({ children }: { children: ReactNode }) {
  ...
  // Low power: no enter/exit animation at all — instant paint, zero layout work.
  if (quality === "low") return <main className="pt-[72px]">{children}</main>;

  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.main
        key={pathname}
        // Opacity + translate only: `filter: blur()` forces a full-page repaint
        // every frame, which is what made page entry feel laggy.
        initial={{ opacity: 0, y: 16 * scale }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -10 * scale }}
        transition={{ duration: quality === "high" ? 0.32 : 0.22, ease: [0.2, 0, 0, 1] }}
