...
export function Marquee({ items }: { items: string[] }) {
  ...
  return (
    <div className="relative flex overflow-hidden border-y border-border bg-surface-1 py-4">
      <motion.div
...
export function Scroll3D({
  ...
  const smooth = useSpring(scrollYProgress, { stiffness: 90, damping: 26, mass: 0.28 });
  const k = intensity * scale;
  const rotateX = useTransform(smooth, [0, 0.5, 1], [11 * k, 0, -9 * k]);
  const y = useTransform(smooth, [0, 0.5, 1], [34 * k, 0, -26 * k]);
  const s = useTransform(smooth, [0, 0.5, 1], [1 - 0.05 * scale, 1, 1 - 0.03 * scale]);
  const opacity = useTransform(smooth, [0, 0.2, 0.85, 1], [0.45, 1, 1, 0.6]);
